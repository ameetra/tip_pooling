...
export async function verifyMagicLink(token: string): Promise<{ jwt: string }> {
  ...
  // Mark used before issuing JWT (prevent race conditions)
  ...
  const employee = await (prisma as any).employee.findFirst({
    where: { email: record.email, isActive: true, isDeleted: false },
  });
  ...
  const signed = jwt.sign(payload, JWT_SECRET, { expiresIn: '24h' });
  return { jwt: signed };
}
